...
> 구현 정합성 안내: 이 문서는 현재 MVP의 안전 정책과 Tool 계약을 기반으로 한 **목표 아키텍처**다. 현재 MVP는 입력·권한·승인·기록 저장을 결정론적 Backend 정책으로 통제하고, OpenAI 모델은 의도 분류와 일반 육아 안내 생성에 사용한다. 아래의 공통 Agent Loop·StateGraph·상세 Event Trace는 이 구현을 확장·정리하는 설계 목표이며, 모든 노드가 별도 Runtime으로 구현된 상태를 뜻하지 않는다.
