...
> 재시도 횟수는 **최초 호출 이후의 추가 호출 횟수**다. 본 보고서의 평균 재시행 횟수는 **Agent 수준의 추가 Tool 실행 횟수**를 기준으로 하며, MCP 서버 내부 또는 외부 HTTP 클라이언트가 자체적으로 수행하는 재시도는 포함하지 않는다. 답변 재구성은 Tool 재호출과 구분하여 Trace의 `strategy`로 별도 기록한다. 저장처럼 상태를 변경하는 Tool은 네트워크 재시도보다 멱등 키 검증을 우선한다.
...
| v0.4 | Live 요청에서 실제 Tool 경로를 Trace만으로 식별할 수 없음 | Tool 사용 요청이 `knowledge_search`로 고정 기록되어 실제 호출 Tool을 구분할 수 없었음 | 실제 Tool명·안전한 인수 요약·결과 검증 상태를 Redis Trace에 기록 | 실제 Tool 선택과 실행 경로를 사후 검증할 수 있도록 관측성을 높이기 위함 | 동일 유형 Live 파일럿 3건에서 일반 안내=Tool 미호출, 병원=`search_pediatric_hospitals`, 수유 RAG=`search_feeding_guide`로 식별 가능함을 확인 |
...
| 실제 도구 선택 정확도 | 측정 불가 | 3건 모두 기대 경로 확인 | 적용 전 Trace가 실제 Tool을 식별하지 못해 동일 기준의 전후 정량 비교 불가 |
| Trace 도구 식별 상태 | 실제 Tool 구분 불가 | 3/3 식별 가능 | 실제 Tool명 또는 Tool 미호출 상태를 Trace에서 확인 가능한 건수 ÷ 3 |
...
적용 전 Trace는 Tool 사용 요청을 모두 `knowledge_search`로 고정 기록했으므로 Agent가 실제로 어떤 Tool을 선택했는지 사후 판정할 수 없다. 따라서 적용 전 값을 `도구 선택 정확도 0%`로 계산하지 않고 **측정 불가**로 처리한다. 적용 후에는 실제 Tool명과 필요한 인수를 기록하므로 3개 시나리오 모두 기대 실행 경로를 Trace에서 식별할 수 있었다. 이는 **Tool 선택 성능이 0%에서 100%로 개선됐다는 뜻이 아니라, 실제 선택을 검증할 수 있는 Trace 관측성이 확보됐다는 의미**다. 세 요청 모두 첫 실행에서 성공하여 Agent 수준 재시도는 발생하지 않았다.
...
| 응답 일관성 | 85% (17/20) | 100% (20/20) | 현재 자동 시험에서 결과 상태·출처·응답 유형이 기대 결과와 일치한 건수 ÷ 전체 20건 |
| 평균 재시행 횟수 | 0.0회 | 0.1회 | 총 재시도 2회 ÷ 20 |

Baseline의 미완료 3건은 병원 Tool 일시 실패, 병원 Tool 재시도 소진, RAG 근거 없음이다. Tool 선택은 두 조건 모두 정책 계획이 이미 정확했으므로 변하지 않았다. 다만 이 20건 시험은 기대 Tool이 포함된 계획을 통제 조건으로 사용하는 평가이므로, **독립적인 자연어 Tool 선택 성능 시험으로 해석하지 않는다.** 자연어 입력에서 실제 계획 함수가 선택한 Route·Tool은 9.4에서 별도로 확인한다. Reflection은 실행 결과 검증과 복구 단계에 적용되어 필요한 경우에만 재시도·안전 종료를 수행한다.

**응답 일관성 측정 범위:** 본 20건 시험의 응답 일관성은 Tool 실행 결과의 성공·실패 상태, RAG 출처 존재 여부, 최종 응답 유형이 기대 결과와 일치하는지를 기준으로 측정하였다. Tool 결과의 수유량·시각·병원명·주소 등 구조화된 실제 값과 최종 답변의 값까지 자동 비교하는 검증은 별도 확대 시험 항목으로 관리한다. 따라서 본 지표의 100%는 **현재 정의한 자동 판정 범위에서의 일관성**을 의미한다.
...
- Reflection 적용 후 모든 지표가 반드시 상승해야 하는 것은 아니다. Reflection이 Tool 선택 이후의 결과 검증·복구에 적용되는 시험에서는 도구 선택 정확도가 동일해도 정상이다.
...
- 응답 일관성은 가이드라인의 지표명을 그대로 사용하되 측정 범위를 명시한다. 현재 20건 Mock의 실측값은 상태·출처·응답 유형의 정합성을 기준으로 하며, 구조화 수치·병원명·주소 등 실제 값 대조는 별도 확대 시험으로 관리한다.
