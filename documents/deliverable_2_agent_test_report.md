...
| 응답 상태·근거 일관성 | 85% (17/20) | 100% (20/20) | 현재 판정기가 확인한 결과 상태·출처·응답 유형의 일치 건수 ÷ 20 |
...
또한 이 표의 `응답 상태·근거 일관성`은 현재 자동 판정기가 확인한 **결과 상태·출처·응답 유형의 정합성**을 의미한다. T-05처럼 Tool 결과가 `100ml`인데 답변 초안을 강제로 `120ml`로 만드는 수치 불일치 주입 후 자동 재구성까지 검증한 지표는 아니다. 구조화 값의 강제 불일치→감지→재구성→재검증은 확대 시험 항목으로 분리하며, 완료 전에는 해당 능력을 100% 실측했다고 표현하지 않는다.
...
- 응답 일관성은 측정 범위를 명시한다. 현재 20건 Mock의 실측값은 상태·출처·응답 유형 정합성이고, 구조화 수치·병원명·주소 등 실제 값 대조는 별도 확대 시험으로 관리한다.
