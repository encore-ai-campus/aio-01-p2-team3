...
현재 Redis Trace의 selected_tools는 일반 육아 안내·병원 검색·RAG 검색을 모두 knowledge_search로 표시한다. 따라서 이번 Trace는 요청 처리 성공과 Trace 저장 여부의 증거로만 사용하며, 도구 선택 정확도와 실행 순서 측정에는 실제 Tool명·인수·결과를 Event 단위로 저장하는 Trace 보강이 필요하다.
