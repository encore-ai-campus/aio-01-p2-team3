...
| v0.4 | Tool 결과와 최종 답변의 불일치 가능성 | 결과 스키마·요청 카테고리·출처·답변 수치를 대조하는 Reflection 검증 단계를 추가 | T-03, T-05에서 잘못된 결과·답변을 차단하는지 측정 |
| v0.5 | 일시 오류 시 반복 호출 위험 | 병원 API와 MCP 연결 오류의 추가 재시도를 각각 1회로 제한하고 종료 사유를 Trace에 기록 | T-04에서 상한 후 안전 종료 여부 확인 |
...
| 태스크 완료율 | 시나리오 Trace 측정 전 | 측정 전 | 완료 케이스 수 ÷ 전체 시험 케이스 수 |
| 도구 선택 정확도 | 측정 전 | 측정 전 | 기대 Tool과 실제 Tool이 일치한 건수 ÷ Tool 선택 대상 건수 |
| 응답 일관성 | 측정 전 | 측정 전 | Tool 결과·출처와 최종 답변이 일치한 완료 건수 ÷ 완료 건수 |
| 평균 재시행 횟수 | 측정 전 | 측정 전 | 추가 실행·답변 재구성 횟수 합계 ÷ 전체 시험 케이스 수 |
