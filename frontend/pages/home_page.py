...
def render() -> None:
    ...
    feeding_interval_label = pattern.get("average_interval", "3시간 12분")
    try:
        hours_text, minutes_text = feeding_interval_label.split("시간", maxsplit=1)
        feeding_interval_minutes = (int(hours_text.strip()) * 60) + int(minutes_text.replace("분", "").strip() or 0)
    except (ValueError, AttributeError):
        feeding_interval_minutes = 192
        feeding_interval_label = "3시간 12분"
